"""Suspension subsystem simulated sensor data generator."""
...
def generate(out_path: str, steps: int = 2000, fault_rate: float = 0.05):
    print(f"Generating Suspension subsystem data -> {out_path}")
    sim = TankSimulator(steps=steps)
    records = sim.run()
    
    # Filter columns relevant to this subsystem
    keep_cols = ['time', 'step', 'suspension_strain', 'suspension_load', 'suspension_stroke', 'torsion_stress']
...
if __name__ == "__main__":
    ...
    parser.add_argument("--out", default="results/simulated/suspension.csv")
