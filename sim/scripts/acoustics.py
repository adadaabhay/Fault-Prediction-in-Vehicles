"""Acoustics subsystem simulated sensor data generator."""
...
def generate(out_path: str, steps: int = 2000, fault_rate: float = 0.05):
    print(f"Generating Acoustics subsystem data -> {out_path}")
    sim = TankSimulator(steps=steps)
    records = sim.run()
    
    # Filter columns relevant to this subsystem
    keep_cols = ['time', 'step', 'vib_accel_x', 'vib_accel_y', 'vib_accel_z', 'ae_event_rate', 'ae_burst_energy', 'spl_db']
...
if __name__ == "__main__":
    ...
    parser.add_argument("--out", default="results/simulated/acoustics.csv")
