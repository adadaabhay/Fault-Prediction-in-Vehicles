"""Engine subsystem simulated sensor data generator."""
...
def generate(out_path: str, steps: int = 2000, fault_rate: float = 0.05):
    print(f"Generating Engine subsystem data -> {out_path}")
    cfg = TankConfig()
    sim = TankSimulator(cfg=cfg, seed=42)
    records = sim.run()
    
    # Filter columns relevant to this subsystem
    keep_cols = ['time', 'step', 'rpm', 'load', 'coolant_temp', 'exhaust_temp', 'shaft_torque', 'oil_pressure', 'oil_temp']
...
if __name__ == "__main__":
    ...
    parser.add_argument("--out", default="results/simulated/engine.csv")
