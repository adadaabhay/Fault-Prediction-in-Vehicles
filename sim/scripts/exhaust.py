"""Exhaust subsystem simulated sensor data generator."""
...
def generate(out_path: str, steps: int = 2000, fault_rate: float = 0.05):
    print(f"Generating Exhaust subsystem data -> {out_path}")
    cfg = TankConfig()
    sim = TankSimulator(cfg=cfg, seed=42)
    records = sim.run()
    
    # Filter columns relevant to this subsystem
    keep_cols = ['time', 'step', 'exhaust_mass_flow', 'exhaust_lambda', 'exhaust_backpressure', 'particulate_index']
...
if __name__ == "__main__":
    ...
    parser.add_argument("--out", default="results/simulated/exhaust.csv")
