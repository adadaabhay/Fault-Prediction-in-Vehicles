...
classes). Combo scenarios are tracked in `per_scenario[].combo` and the
combo-derived windows inherit the alphabetically-first observable label, so
the operator sees the co-fault in the per-scenario metadata and the
classification head still corresponds to the declared fault taxonomy. A
multi-label reformulation would change the loss and the head, and is out of
